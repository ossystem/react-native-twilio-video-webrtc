...
RCT_EXPORT_METHOD(startLocalVideo:(BOOL)screenShare) {
  if (screenShare) {
    ...
    self.localScreenTrack = [TVILocalVideoTrack trackWithCapturer:self.screen enabled:NO constraints:[self videoConstraints] name:@"screen"];
  }
...
RCT_EXPORT_METHOD(connect:(NSString *)accessToken roomName:(NSString *)roomName) {
  TVIConnectOptions *connectOptions = [TVIConnectOptions optionsWithToken:accessToken block:^(TVIConnectOptionsBuilder * _Nonnull builder) {
    ...
    // codec preference
    builder.preferredVideoCodecs = @[ [TVIH264Codec new], [TVIVp8Codec new], [TVIVp9Codec new] ];
  }];
